#!/usr/bin/env bash
# =============================================================================
#  benchmark_batch.sh - roda inferencia repetida sobre um lote FIXO de frames
...
#
#  EXTRAIDO de monitor.sh (que agora e' so' um visualizador ao vivo, sem
#  gerar nenhum CSV - ver monitor.sh) pra nao perder essa capacidade de
#  benchmark em lote. Mesma logica de sempre, sem mudanca de comportamento -
#  plot_results.py continua lendo experiments/<MODEL_NAME>.csv exatamente
#  como antes.
#
#  So' pro ambiente board_test/ (via qnn-net-run). NAO existe equivalente
#  valido pro native_infer/ neste formato de "N iteracoes sobre lote fixo":
#  desde que native_infer/qnn_infer ganhou o modo --loop (carrega o modelo 1x,
#  executa multiplos frames), a forma correta de medir FPS real dele e' via
#  board_test/03_live_loop.py (uso continuo real, nao um lote repetido) - ver
#  native_infer/README.md.
#
...
# =============================== CONFIG ======================================
MODEL_NAME="260420_1280_large"                   # so' um rotulo p/ nome do CSV
BIN_PATH="../models/260420_1280_large.bin"       # .bin a testar (dir compartilhado)
...
echo "[benchmark] capturando lote de frames (1x, reusado em todas as $ITERATIONS iteracoes)..."
...
for i in $(seq 1 "$ITERATIONS"); do
  TMPOUT="/tmp/benchmark_out_$$"
  TMPLOG="/tmp/benchmark_log_$$.txt"
  stat0=$(read_cpu_stat)
  t0=$(date +%s.%N)
  ...
  if [ $ret -ne 0 ]; then
    echo "[benchmark] iteracao $i FALHOU (qnn-net-run retornou $ret):"
    cat "$TMPLOG" >&2
    rm -f "$TMPLOG"
    continue
  fi
  ...
  echo "[benchmark] $i/$ITERATIONS: ${fps} FPS  cpu=${cpu}%  ram=${ram}MB  temp_cpu=${tcpu}C  temp_npu=${tnpu}C  cdsp=${cdsp}"
done

echo "[benchmark] concluido -> $CSV"
